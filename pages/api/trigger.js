  ...
  for (const { name, fn } of SOURCES) {
    ...
    try {
      ...
      if (!rows.length) {
        ...
        results.push({ source: name, status: 'no_data', fetched: 0, ms: Date.now()-t });
        continue;
      }
      const { count } = await upsertViolations(rows);
      newStored += count ?? 0;
      await logCron(name, rows.length, 'ok');
      results.push({ source: name, status: 'ok', fetched: rows.length, new: count??0, ms: Date.now()-t });
    } catch(e) {
...
h2{font-size:14px;font-weight:600;margin-bottom:14px;color:#1a1814}
.big{font-size:44px;font-weight:700;color:#8b6914;line-height:1;margin-bottom:4px}
...
.btn{display:inline-block;margin-top:20px;background:#1a1814;color:#f8f6f1;padding:12px 28px;border-radius:6px;text-decoration:none;font-size:14px;font-weight:500}
</style></head><body>
...
<div class="card">
  ...
  <div style="font-size:13px;color:#9a9488">Total records stored</div>
  <div class="note">+${newStored} new this run · duplicates skipped automatically</div>
</div>
...
<div class="card">
  ...
  ${results.map(r=>`<div class="row">
    ...
    <span class="${r.status==='ok'?'ok':r.status==='no_data'?'warn':'bad'}">
      ${r.status==='ok'
        ?`✓ ${r.fetched} fetched · ${r.new} new · ${r.ms}ms`
        :r.status==='no_data'
        ?`⚠ No data · ${r.ms}ms`
        :`✗ ${(r.error||'').slice(0,70)} · ${r.ms}ms`}
    </span>
  </div>`).join('')}
</div>
