...
export async function fetchDOL(apiKey) {
  ...
  for (let offset = 0; offset < 5000; offset += 1000) {
    try {
      const url = `https://apiprod.dol.gov/v4/get/WHD/whd_whisard?limit=1000&offset=${offset}`;
...
export async function fetchUKHMRC() {
  ...
  // Direct asset URLs — newest first.
  // Skip the Content API: gov.uk returns 403 for Vercel server IPs.
  const URLS = [
...
export async function fetchAustralia() {
  ...
  // Source B: Media releases — scan recent enforcement articles
  try {
...
export async function fetchIreland() {
  ...
  for (const url of PAGES) {
    ...
    try {
      ...
      // Regex for inline enforcement mentions with amounts
      const body = htmlText(html);
      const rx   = /([A-Z][A-Za-z0-9\s&'(),-]{3,60}?)\s+(?:ordered|fined|penalised|awarded)[^.]{0,150}[€£]([\d,]+)/g;
      let m;
      while ((m = rx.exec(body)) !== null) {
        const name = sanitize(m[1].trim().replace(/^(The|A|An)\s+/i, ''));
        if (!name || name.length < 3) continue;
        rows.push({
          ...
          employees_affected: 0,
          amount_back_wages:  toFloat(m[2].replace(/,/g, '')),
          amount_penalties:   0,
          country:            'Ireland',
          year:               new Date().getFullYear(),
...
export async function fetchNetherlands() {
  ...
  for (const listUrl of PAGES) {
    ...
    try {
      ...
      // Get article links
      const linkRx = /href="(\/actueel\/nieuws\/[^"]+|\/onderwerpen\/[^"]+\/nieuws\/[^"]+)"/gi;
      const links  = [...new Set([...listHtml.matchAll(linkRx)].map(m => m[1]))].slice(0, 12);
    ...
    try {
      ...
      for (const path of links) {
        try {
          ...
          // ELA mostly covers cross-border enforcement — extract employer names + amounts
          const rx = /([A-Z][A-Za-z0-9\s&'(),-]{3,60}?)\s+(?:penalised|fined|sanctioned|infringed|ordered to pay)[^.]{0,120}(?:€|EUR)\s*([\d,.]+)/gi;
          let m;
          while ((m = rx.exec(body)) !== null) {
            const name   = sanitize(m[1].trim().replace(/^(The|A|An)\s+/i, ''));
