...
export async function fetchDOL(apiKey) {
  const rows = [];
  if (!apiKey) {
    console.error('[DOL] ✗ DOL_API_KEY env var is not set — skipping');
    return rows;
  }
  ...
  for (let skip = 0; skip < 5000; skip += PAGE) {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        console.error(`[DOL] ✗ HTTP ${res.status} — API key may be invalid or expired. Get a new key at developer.dol.gov`);
        break;
      }
      ...
      if (text.trim().startsWith('<')) {
        console.error(`[DOL] ✗ Got HTML response — API key is invalid or expired. Check DOL_API_KEY env var.`);
        break;
      }
      ...
      for (const r of records) {
        const bw = toFloat(r.bw_atp_amt ?? r.flsa_bw_atp_amt ?? r.bw_amt ?? 0);
        if (bw <= 0) continue;
        const name = sanitize(r.legal_name ?? r.trade_nm ?? r.employer_name ?? '');
        if (!name) continue;
        rows.push({
          company_name:       name,
          trade_name:         sanitize(r.trade_nm ?? null),
          industry:           sanitize(r.naic_description ?? r.naics_description ?? null),
          violation_type:     dolType(r),
          employees_affected: toInt(r.cmp_ee_atp_cnt ?? r.ee_violtd_cnt ?? 0),
          amount_back_wages:  bw,
          amount_penalties:   toFloat(r.ee_atp_amt ?? r.civil_money_penalty ?? 0),
